BetaJS.UI.Interactions.ElementInteraction.extend("BetaJS.UI.Interactions.Drag", {
	...
	__triggerDomEvent: function (label) {
		...
		if (underneath) {
			underneath.dispatchEvent(new CustomEvent("drag-" + label, {
				detail: data,
				bubbles: true,
				cancelable: true
			}));
		}
		if (this.__underneath && this.__underneath != underneath) {
			this.__underneath.dispatchEvent(new CustomEvent("drag-leave", {
				detail: data,
				bubbles: true,
				cancelable: true
			}));
...
BetaJS.UI.Interactions.Drag.Idle.extend("BetaJS.UI.Interactions.Drag.Stopping", {
	...
	_locals: ["page_coords", "initial_element_coords", "cloned_element", "cloned_modifier", "immediately"],
	
	_start: function () {
		this.trigger("stopping");
		this.triggerDom("leave");
